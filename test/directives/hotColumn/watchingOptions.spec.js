...
describe('hotColumn', function () {
  ...
  it('should create table with `numericFormat` attribute in columns', function () {
    rootScope.formats = [void 0, '0,00', '0.0USD'];
    var scope = angular.element(compile(
      "<hot-table>" +
      "<hot-column type='numeric' numeric-format='{pattern: formats[0]}'></hot-column>" +
      "<hot-column type='numeric' numeric-format='{pattern: formats[1]}'></hot-column>" +
      "<hot-column type='numeric' numeric-format='{pattern: formats[2]}'></hot-column>" +
      "</hot-table>"
    )(rootScope)).isolateScope();
    ...
    expect(columns[0].numericFormat.pattern).toBe(void 0);
    expect(columns[1].numericFormat.pattern).toBe('0,00');
    expect(columns[2].numericFormat.pattern).toBe('0.0USD');
  });
